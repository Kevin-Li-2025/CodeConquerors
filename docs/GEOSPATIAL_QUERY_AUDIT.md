...
| Route graph load | `RouteGraphRepository.LoadGraphAsync` | `route_edges` bbox scan on imported OSM graph can grow quickly. | `IX_route_edges_geometry_gist` supports `ST_Intersects`; runtime caps `Routing__MaxRouteGraphEdges`, caches compressed packed prepartitioned shards, runs ALT preprocessing for non-truncated shards, and can use `READONLY_DATABASE_URL` for hot read traffic. The offline OSM extract profiler validates shard hit ratio and artifact size before PostGIS becomes the measurement bottleneck. |
| Route edge cost | OSM import -> worker A* | Recomputing surface/smoothness/width/kerb penalties during every edge expansion burns CPU on large shards. | OSM import stores versioned standard/wheelchair/stroller accessibility penalties on `route_edges`; packed graph artifacts also carry versioned static traversal weights. |
...
- Run `EXPLAIN (ANALYZE, BUFFERS)` and `tools/profile-city-route-graph.sh` against production-sized OSM extracts before launch.
